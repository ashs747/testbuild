...
then
  ...
  rm $2.zip
  aws elasticbeanstalk create-application-version --application-name "React Reflux Skeleton" --version-label $2 --source-bundle S3Bucket=cirrus-deployments,S3Key=react-reflux-skeleton/$2.zip
